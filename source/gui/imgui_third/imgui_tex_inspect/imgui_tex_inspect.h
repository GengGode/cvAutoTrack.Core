...
enum InspectorFlags_
{
    ...
    InspectorFlags_RotateRight          = 1 << 9,  // Rotate the texture 90 degrees clockwise
    InspectorFlags_RotateLeft           = 1 << 10, // Rotate the texture 90 degrees counter-clockwise
};
...
bool BeginInspectorPanel(Context* ctx,const char *name, ImTextureID, ImVec2 textureSize, InspectorFlags flags = 0);
bool BeginInspectorPanel(Context* ctx,const char *name, ImTextureID, ImVec2 textureSize, InspectorFlags flags, SizeIncludingBorder size);
bool BeginInspectorPanel(Context* ctx,const char *name, ImTextureID, ImVec2 textureSize, InspectorFlags flags, SizeExcludingBorder size);
 ...
 */
template <typename T>
void DrawAnnotations(Context* ctx, T drawer, ImU64 maxAnnotatedTexels = 0);
...
class ValueText
{
  ...
  public:
    ...
    void DrawAnnotation(Context* ctx, ImDrawList *drawList, ImVec2 texel, Transform2D texelsToPixels, ImVec4 value);
};
...
bool GetAnnotationDesc(Context* ctx, AnnotationsDesc *, ImU64 maxAnnotatedTexels);

//-------------------------------------------------------------------------
// [SECTION] TEMPLATE IMPLEMENTATION
//-------------------------------------------------------------------------
template <typename T>
void DrawAnnotations(Context* ctx, T drawer, ImU64 maxAnnotatedTexels)
{
    AnnotationsDesc ad;
    if (GetAnnotationDesc(ctx, &ad, maxAnnotatedTexels))
    {
        ...
        for (int ty = (int)ad.TexelTopLeft.y; ty < (int)texelBottomRight.y; ++ty)
        {
            for (int tx = (int)ad.TexelTopLeft.x; tx < (int)texelBottomRight.x; ++tx)
            {
                ...
                drawer.DrawAnnotation(ctx, ad.DrawList, center, ad.TexelsToPixels, color);
            }
        }
    }
}
