...
void DestroyContext(Context *ctx)
{
    ...
    for (auto &pair : ctx->Inspectors.Data)
    {
...
bool BeginInspectorPanel(Context* ctx, const char *title, ImTextureID texture, ImVec2 textureSize, InspectorFlags flags,
                         SizeIncludingBorder sizeIncludingBorder)
{
    ...
    if (HasFlag(flags, InspectorFlags_RotateRight) || HasFlag(flags, InspectorFlags_RotateLeft))
    {
        ImSwap(textureSize.x, textureSize.y);
    }
    ...
    if (HasFlag(flags, InspectorFlags_RotateRight) || HasFlag(flags, InspectorFlags_RotateLeft))
        ImSwap(viewSizeUV.x, viewSizeUV.y);
    ImVec2 uv0 = inspector->PanPos - viewSizeUV * 0.5;
    ImVec2 uv1 = inspector->PanPos + viewSizeUV * 0.5;
    if (HasFlag(flags, InspectorFlags_RotateRight) || HasFlag(flags, InspectorFlags_RotateLeft))
    {
        ImVec2 PanPos = ImVec2(inspector->PanPos.y, inspector->PanPos.x);
        uv0 = PanPos - viewSizeUV * 0.5;
        uv1 = PanPos + viewSizeUV * 0.5;
    }
    ...
    if (ImGui::BeginChild(title, panelSize, false, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoMove))
    {
        ...
        //if(0)
        {
            int angle = HasFlag(inspector->Flags, InspectorFlags_RotateRight) ? 90 : 0 - HasFlag(inspector->Flags, InspectorFlags_RotateLeft) ? 90 : 0;
            if (angle == 0)
            {
                ImGui::Image(texture, viewSize, uv0, uv1);
            }
			...
			{
                auto ImageRotate = [](ImTextureID user_texture_id, const ImVec2& image_size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1))
                    {
                        ImGuiWindow* window = ImGui::GetCurrentWindow();
                        ...
                        ImGui::ItemAdd(bb, 0);
                        ImVec2 pos[4] = {
                            ImVec2(bb.Min.x, bb.Min.y),   // 1 4
                            ImVec2(bb.Max.x, bb.Min.y),   // 2 1
                            ImVec2(bb.Max.x, bb.Max.y),   // 3 2
                            ImVec2(bb.Min.x, bb.Max.y) }; // 4 3
                        ImVec2 uvs[4] =
                        {
                            uv0,
                            ImVec2(uv1.x, uv0.y),
                        ...
                        window->DrawList->AddImageQuad(user_texture_id, pos[1], pos[2], pos[3], pos[0], uvs[0], uvs[1], uvs[2], uvs[3], IM_COL32_WHITE);
                    };
                ImageRotate(texture, viewSize, uv0, uv1);
			}
			else if (angle == -90)
			{
                auto ImageRotate = [](ImTextureID user_texture_id, const ImVec2& image_size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1))
					{
						...
						ImGui::ItemAdd(bb, 0);
						ImVec2 pos[4] = {
							ImVec2(bb.Min.x, bb.Min.y),   // 1 2
							ImVec2(bb.Max.x, bb.Min.y),   // 2 3
							ImVec2(bb.Max.x, bb.Max.y),   // 3 4
							ImVec2(bb.Min.x, bb.Max.y) }; // 4 1
						ImVec2 uvs[4] =
						{
							uv0,
							ImVec2(uv1.x, uv0.y),
						...
						window->DrawList->AddImageQuad(user_texture_id, pos[3], pos[0], pos[1], pos[2], uvs[0], uvs[1], uvs[2], uvs[3], IM_COL32_WHITE);
					};
				ImageRotate(texture, viewSize, uv0, uv1);
			}
        }
        ...
        {  //DRAGGING
            ...
            else if (inspector->IsDragging)
            {
                ...
                if(HasFlag(inspector->Flags, InspectorFlags_RotateRight) || HasFlag(flags, InspectorFlags_RotateLeft))
				{
					uvDelta.x *= -1;
				}
...
bool BeginInspectorPanel(Context* ctx,const char *name, ImTextureID texture, ImVec2 textureSize, InspectorFlags flags)
{
    return BeginInspectorPanel(ctx, name, texture, textureSize, flags, SizeIncludingBorder{{0, 0}});
}

bool BeginInspectorPanel(Context* ctx, const char *name, ImTextureID texture, ImVec2 textureSize, InspectorFlags flags, SizeExcludingBorder size)
{
    // Correct the size to include the border, but preserve 0 which has a special meaning
    return BeginInspectorPanel(ctx, name, texture, textureSize, flags,
                               SizeIncludingBorder{ImVec2{size.size.x == 0 ? 0 : size.size.x + 2, 
                                                          size.size.y == 0 ? 0 : size.size.y + 2}});
}

void EndInspectorPanel(Context* ctx)
{
...
void SetScaleMax(Inspector* inspector, ImVec2 scale)
{
    inspector->ScaleMax = scale;
}

void SetScaleMin(Inspector* inspector, ImVec2 scale)
{
    inspector->ScaleMin = scale;
}
...
Inspector* GetCurrentInspector(const Context* ctx)
{
    return ctx->CurrentInspector;
}
 ...
 */
bool GetAnnotationDesc(Context* ctx, AnnotationsDesc *ad, ImU64 maxAnnotatedTexels)
{
    Inspector *inspector = ctx->CurrentInspector;
...
void ValueText::DrawAnnotation(Context*, ImDrawList *drawList, ImVec2 texel, Transform2D texelsToPixels, ImVec4 value)
{
    char buffer[64];
